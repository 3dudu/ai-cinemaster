...
- ✨ 新增智能图片处理功能：自动将 HTTP/HTTPS URL 转换为 Base64
  - 文生图支持远程参考图 URL 自动转换
  - 图生视频支持起始帧/结束帧 URL 自动转换
  - 支持角色，场景，分镜编辑修改
  - 支持本地上传图片
  - 支持项目导入导出
- 🐛 优化模型提供商选择默认值逻辑
- 📝 完善 README 文档，添加工具函数说明
- 📱 初步支持移动端使用
...
- 📦 **资源导出**: 一键导出所有关键帧和视频资源
- 🔄 **智能图片处理**: 自动将 HTTP/HTTPS URL 转换为 Base64，支持远程参考图无缝使用
...
工具函数
├── imageUtils.ts (图片工具: URL 转 Base64、Base64 转 Blob)
├── fileUploadUtils.ts (文件上传工具: 本地服务器文件上传)
...
**Q: HTTP/HTTPS 图片 URL 无法作为参考图?**
A: 系统会自动将 HTTP/HTTPS URL 转换为 Base64 格式，如果转换失败会继续使用原始 URL。请检查网络连接和 URL 的可访问性
