...
5. 判定场景动态：是“文戏（需微操化，如情绪细节）”还是“武戏（保留大动态，配合参考素材）”。
...
9. 台词与节奏：说明语气和语速，声音大小。台词数量与语速需适配{segmentDuration}s时长，确保每句台词完整、问答间有自然停顿，避免语速过快或超时说不完，台词前需描述角色语气，台词用「」包围。
10. 时间轴分段灵活：按剧情节奏自然划分，不强制按分镜时间设定，可自行调整，总时长不超过{segmentDuration}s。
11. 描述要自然流畅，符合电影叙事逻辑和拍摄手法，不改变分镜原意，不添加额外内容。
