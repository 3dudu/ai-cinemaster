  ...
  switch (key) {
    ...
    case 'GENERATE_SCRIPT':
      return {
        ...
        lang: args[3] || '中文',
        story: args[4] || '',
        episodeCount: args[5] || 10,
        totalDuration: args[6] || ''
      };
...
本集的故事高层概括，包含：主要冲突、关键转折、情感弧线，200-300字。一句话的集定位和情绪曲线。
...
### 剧情梗概
挖墙的男孩，一个普通的男孩，在一次挖墙中失去了生命。

**集定位：** 至暗时刻——主角失去一切，被逼到绝境
**情绪曲线：** 压抑→绝望→虚假希望→信任崩塌→彻底崩溃→重新站起
...
## 强制要求
  ...
  - 每一段为4-5个镜头组组成,分镜头要具备足够的导演思维，完美兼顾人物动作、情绪高潮等镜头组的衔接。
  - 必须将剧本中的台词精准分配到对应的镜头里，台词用「」包围。
  - 遇到情绪高潮点，多使用特写或推镜头；遇到战斗场面，请增加镜头密度，并极度细化战斗动作、武器碰撞反馈和运镜方式。
  - 所有画面、人物、动作描述的文字严禁出现任何敏感词、违禁词或可能触发审核的内容。
...
5. 每一段为4-5个镜头组组成,分镜头要具备足够的导演思维，完美兼顾人物动作、情绪高潮等镜头组的衔接。
6. 必须将剧本中的台词精准分配到对应的镜头里，台词用「」包围。
7. 遇到情绪高潮点，多使用特写或推镜头；遇到战斗场面，请增加镜头密度，并极度细化战斗动作、武器碰撞反馈和运镜方式。
8. 分镜头要具备足够的导演思维，完美兼顾人物动作、情绪高潮等镜头组的衔接。`,
...
{scriptText}`,
  ...
  // ============ 剧本生成 ============
  GENERATE_SCRIPT: `
核心任务：结合你的专业能力，把下面的小说/故事内容改编扩写成一个 {genre} 题材类型的剧本，总时长约 {totalDuration} ，一共 {episodeCount} 集、每集 {duration} 的短剧文学剧本。
...
10. 单集时长控制在【项目配置】指定值 ±10秒，台词量按 150字/分钟 推算（禁止硬编码）
...
## 硬性输出规则（必须严格遵守）
1. 按照单集时长分成 4-6 个片段,每个片段长不超过 {segmentDuration} 秒。
2. 在每个片段中完成独立的叙事，内容完整，结构统一。台词较多时应拆分多个片段。
3. 每一段为4-5个镜头组组成,分镜头要具备足够的导演思维，完美兼顾人物动作、情绪高潮等镜头组的衔接。
4. 必须将剧本中的台词精准分配到对应的镜头里，台词用「」包围。
5. 遇到情绪高潮点，多使用特写或推镜头；遇到战斗场面，请增加镜头密度，并极度细化战斗动作、武器碰撞反馈和运镜方式。
...
2. 输出干净整洁，可直接复制生成视频。
3. 负面提示词固定不变，每个片段必须带。
4. 输出前需要检查每一集第一个片段与上一集最后一个片段在剧情上，是否有重复，是否连贯，如果有问题，修改后在输出。
5. 所有描述文字不得出现敏感词、暴力、血腥、政治、色情等内容。
...
- 单片段时长不超过 {segmentDuration} ，所有片段的 estimatedDuration 和必须大于 {targetDuration}

## 输出格式
- 输出JSON格式，包含所有片段的分时描述，以及每个片段的 estimatedDuration 和 motionIntensity 等信息
- 输出JSON格式如下：
\`\`\`json
{
  "segments": [
...
9. 每一段为4-5个镜头组组成,分镜头要具备足够的导演思维，完美兼顾人物动作、情绪高潮等镜头组的衔接。
10. 必须将剧本中的台词精准分配到对应的镜头里，台词用「」包围。
11. 遇到情绪高潮点，多使用特写或推镜头；遇到战斗场面，请增加镜头密度，并极度细化战斗动作、武器碰撞反馈和运镜方式。
