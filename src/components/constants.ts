...
export const STYLES = {
  ...
  // 卡片样式
  card: {
    base: "p-5 bg-[var(--bg-surface)] border border-slate-600 rounded-xl hover:border-[var(--border-secondary)] transition-colors group cursor-pointer flex flex-col justify-between h-32 relative overflow-hidden",
  ...
  modal: {
    ...
    container: "bg-[var(--bg-surface)] border border-slate-600 rounded-xl max-w-4xl w-full max-h-[80vh] flex flex-col shadow-2xl",
    header: "p-6 border-b border-slate-600 flex items-center justify-between",
    content: "flex-1 overflow-y-auto p-6 space-y-2",
    footer: "p-4 border-t border-slate-600 bg-[var(--bg-primary)] flex justify-end items-center"
  },
  
  // 视频播放器模态框
  videoModal: {
    overlay: "fixed inset-0 bg-[var(--bg-base)]/95 backdrop-blur-sm flex items-center justify-center z-50 p-4",
    container: "bg-[var(--bg-primary)] border border-slate-600 rounded-xl max-w-6xl w-full flex flex-col shadow-2xl overflow-hidden",
    player: "bg-[var(--bg-base)] relative flex items-center justify-center overflow-hidden",
    controls: "p-4 border-t border-slate-600 bg-[var(--bg-primary)] flex items-center justify-between shrink-0"
  },
  
  // 状态面板
  statusPanel: {
    container: "bg-[var(--bg-surface)] border border-slate-600 rounded-xl p-8 shadow-2xl relative overflow-hidden group",
    decoration: {
  ...
  // 时间线
  timeline: {
    container: "h-20 bg-[var(--bg-sunken)] rounded-lg border border-slate-600 flex items-center px-2 gap-1 overflow-x-auto custom-scrollbar relative shadow-inner",
    ...
    segmentIncomplete: "bg-[var(--bg-elevated)] border border-slate-600 hover:bg-[var(--bg-hover)]",
    tooltip: "absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block z-20 whitespace-nowrap"
  },
  
  // 日志项
  logItem: {
    container: "bg-[var(--bg-primary)] border border-slate-600 rounded-lg overflow-hidden hover:border-[var(--border-secondary)] transition-colors",
    header: "p-4 cursor-pointer",
    details: "px-4 pb-4 border-t border-slate-600 pt-3 space-y-3"
  },
