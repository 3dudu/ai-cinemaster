...
const Sidebar: React.FC<SidebarProps> = ({ currentStage, setStage, onExit, onOpenSettings, onToggleSidebar, collapsed = false, projectName, project, updateProject, isSeriesMode = false, onOpenSeriesManager,serieName }) => {
  ...
  return (
    <aside className={`${collapsed ? 'w-20' : 'lg:w-72 w-20'} bg-slate-800 border-r border-slate-600 h-full fixed left-0 top-0 flex flex-col z-60 select-none
    shadow-2xl animate-in slide-in-from-right-10 duration-300 transition-all ease-in-out`}>
      ...
      <button
        ...
        className="hidden xl:block absolute -right-4 top-9.5 bg-slate-800 border border-slate-600 text-slate-400 hover:text-slate-50 hover:bg-slate-700 transition-all rounded-full p-2 z-50 cursor-pointer"
