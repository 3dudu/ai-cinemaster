...
import { ArrowRightLeft, Cloud, Download, Images, Loader2, NotebookPen, RotateCcw, Search, Trash2, Upload, X } from 'lucide-react';
...
export interface ImageLibraryProps {
  ...
  showFooter?: boolean;
  /** 选择后是否自动关闭（默认 false，用于 Modal 模式） */
  closeOnSelect?: boolean;
  /** 是否为 Modal 模式（自动隐藏/显示特定 UI） */
  modalMode?: boolean;
}
...
const ImageLibrary: React.FC<ImageLibraryProps> = ({
  ...
  // 默认标题栏（用于 Modal 模式）
  const renderDefaultTitleBar = () => (
    <div className={`h-14 px-4 md:px-6 border-b border-slate-600 flex items-center justify-between shrink-0 ${modalMode ? 'bg-slate-600/80' : 'bg-slate-700'}`}>
  ...
  return (
    <>
          ...
          <div className="flex items-center gap-3">
            ...
            {remoteImageCount > 0 && (
              <button
                ...
                className="px-3 py-1.5 bg-orange-600 text-white rounded-lg hover:bg-orange-500 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 text-sm"
              >
                {uploadingStatus ? (
                  <>
                    <Loader2 className="w-3 h-3 animate-spin" />
      ...
      <div className="sticky top-0 z-20 p-1 border-b border-slate-600 bg-slate-700">
        <div className="bg-slate-700 rounded-xl p-1 md:px-6 px-1">
          ...
          <div className="flex items-center gap-3">
            ...
            {remoteImageCount > 0 && (
              <button
                onClick={handleBatchUpload}
                disabled={!!uploadingStatus}
                className="px-3 py-1.5 bg-orange-600 text-white rounded-lg hover:bg-orange-500 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
              >
                {uploadingStatus ? (
                  <>
                    <Loader2 className="w-3 h-3 animate-spin" />
